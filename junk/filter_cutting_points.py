...
def filter_cutting_points(skeleton, word, sr, baseLine, maxTransitionIndex, most_frequent_value, most_frequent_value_after_0, vp, hp, second):
    ...
    sr.append([word.shape[1] - 1, word.shape[1] - 1, word.shape[1] - 1, word.shape[1] - 1])
    valid_sr=[]
    top_pixel_in_word_index = get_heighest_pixel_index(hp)
    sr.reverse()
    # print(sr,len(sr))
    ...
    while i < len(sr) - 1:
        ...
        # might need to change operator
            ...
            if (i != len(sr) - 2 and (vp[sr[i+1][3]] == 0 or get_path_cost(skeleton, word, maxTransitionIndex, sr[i+1][1], sr[i+1][0]) > 1000000000000000000) and top_pixel_in_word_index + 16 >= get_heighest_pixel_index(horizintal_projection(word[0:word.shape[0], sr[i+1][3]:sr[i][3]])))\
            ...
            # elif top_pixel_in_word_index + 15 >= get_heighest_pixel_index(horizintal_projection(word[0:word.shape[0], sr[i+1][3]:sr[i][3]])):
            #     valid_sr.append(sr[i])
            #     i += 1
